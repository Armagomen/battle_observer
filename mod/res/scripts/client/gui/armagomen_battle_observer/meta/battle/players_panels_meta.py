...
class PlayersPanelsMeta(BaseModMeta):
    ...
    def as_AddPPanelBarS(self, vehID, color, settings, teamName, startVisible):
        return self.flashObject.as_AddPPanelBar(vehID, color, settings, teamName, startVisible) if self._isDAAPIInited() else None
    ...
    def as_setVehicleIconColorS(self, vehID, color, multipler, enemy):
        return self.flashObject.as_setVehicleIconColor(vehID, color, multipler, enemy) if self._isDAAPIInited() else None
