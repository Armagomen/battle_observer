...
def as_rowCountS(base, carousel, row_count):
    if user_settings.tank_carousel[GLOBAL.ENABLED] and carousel.getAlias() == HANGAR_ALIASES.TANK_CAROUSEL:
        row_count = max(user_settings.tank_carousel[CAROUSEL.ROWS], row_count)
...
def as_setSmallDoubleCarouselS(base, carousel, small):
    return base(carousel,
                small or user_settings.tank_carousel[GLOBAL.ENABLED] and user_settings.tank_carousel[CAROUSEL.SMALL])
