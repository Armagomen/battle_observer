...
for key, value in gm_factory._GUN_MARKER_LINKAGES.items():
    if debug_const in key:
        gm_factory._GUN_MARKER_LINKAGES[key] = value.replace(debug_const, '')
...
class DispersionCircle(object):
    ...
    def createOverrideComponents(base, *args):
        ...
        factory = gm_factory._GunMarkersFactories(gm_factory._DevControlMarkersFactory, gm_factory._OptionalMarkersFactory,
                                                  gm_factory._EquipmentMarkersFactory)
        return factory.create(*args) if len(args) == 2 else factory.override(*args)
