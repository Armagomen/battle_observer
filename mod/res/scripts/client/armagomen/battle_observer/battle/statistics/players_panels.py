...
class PlayersPanelsStatistic(StatsMeta):
    ...
    def updateVehicleData(self, isEnemy, vehicleID):
        ...
        vehicleIconColor = self.py_getIconColor(vInfo.vehicleType.classTag)
        if accountDBID not in self.cache:
            result = getStatisticString(accountDBID, vInfo.player.clanAbbrev)
            if accountDBID and result is not None:
                ...
                self.cache[accountDBID] = (patternFUL % result, patternCUT % result, result[self.COLOR_WTR])
            else:
                self.cache[accountDBID] = (None, None, None)
        self.as_updateVehicleS(isEnemy, vehicleID, vehicleIconColor, *self.cache[accountDBID])
