...
class WGRAndIcons(BaseModMeta):
    ...
    def _populate(self):
        ...
        if region is not None and self.settings[STATISTICS.STATISTIC_ENABLED]:
            ...
            self.data_loader.onDataResponded += self.update_wgr_data
            self.data_loader.getStatisticsDataFromServer()

    def _dispose(self):
        if self.settings[STATISTICS.STATISTIC_ENABLED] and self.data_loader is not None:
            self.data_loader.onDataResponded -= self.update_wgr_data
        self.data_loader = None
...
class StatisticsDataLoader(object):
    ...
    def __init__(self, arena, arenaDP):
        ...
        self.__vehicles = set()
        self.onDataResponded = SafeEvent()
